...
struct ContentView: View {
      ...
      MagazineView(
        ...
        ForEach(Array(items.enumerated()), id: \.element.id) { index, item in
          MagazineItem(
            ...
            totalItemsInSection: itemsInSection(item.section)
          ) {
            ItemCard(item: item)
          }
        }
  ...
  private func itemsInSection(_ section: Int?) -> Int {
    guard let section = section else { return items.count }
    return items.filter { $0.section == section }.count
  }

  private func deleteSelectedItems() {
    withAnimation {
      items.removeAll { selection.selectedItems.contains($0.id) }
      selection.exitSelectionMode()
    }
  }
}
